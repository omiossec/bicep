        ...
        private TypeSymbol ToTypeSymbol(Types.Concrete.TypeBase typeBase)
        {
            switch (typeBase)
            {
                ...
                case Types.Concrete.ResourceType resourceType:
                {
                    ...
                    var type = properties["type"]?.Type?.Type as Types.Concrete.StringLiteralType ?? throw new ArgumentException();
                    var apiVersion = properties["apiVersion"]?.Type?.Type as Types.Concrete.StringLiteralType ?? throw new ArgumentException();
                    var resourceTypeReference = ResourceTypeReference.TryParse($"{type.Value}@{apiVersion.Value}") ?? throw new ArgumentException();
