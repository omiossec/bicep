...
namespace Bicep.Core.Syntax
{
    public class SeparatedSyntaxList : SyntaxBase
    {
        public SeparatedSyntaxList(IEnumerable<SyntaxBase> elements, IEnumerable<Token> separators, TextSpan span)
        {
            ...
            {
                if (this.Elements.Length != this.Separators.Length + 1)
                {
                    throw new ArgumentException($"The number of elements ({this.Elements.Length}) must always be one more than the number of separators ({this.Separators.Length}).");
                }
